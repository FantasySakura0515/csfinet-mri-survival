...
def test_completed_inference_overrides_stale_running_launcher(tmp_path):
    runs = tmp_path / "runs"
    runs.mkdir()
    (runs / "segmentation-suite-v2.json").write_text('{"status":"running"}')
    for name in ("csfinet", "unet"):
        folder = tmp_path / "results/segmentation" / f"{name}-test-v2"
...
def test_missing_or_invalid_manifest_is_not_reported_as_training(tmp_path):
    ...
    folder = tmp_path / "results/segmentation/csfinet-test-v2"
    folder.mkdir(parents=True)
