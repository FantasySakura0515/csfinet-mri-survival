...
def test_release_separates_reports_from_large_artifacts(tmp_path):
    ...
    output = root / "bundle"
    result = create_release(root, "study", output)
    assert result["counts"]["report_files"] == 1
    assert result["counts"]["source_files"] == 2
    with zipfile.ZipFile(output / "csfinet-repro-source-study.zip") as archive:
        assert not any(name.endswith(".pdf") for name in archive.namelist())
    assert result["counts"]["external_artifacts"] == 1176
    with zipfile.ZipFile(output / "csfinet-repro-reports-study.zip") as archive:
        assert archive.namelist() == ["results/tables/study/table.csv"]
    ...
    assert set(manifest["artifacts"]) == {
        "csfinet-repro-source-study.zip", "csfinet-repro-reports-study.zip", "external-artifacts.csv",
        "delivery-audit.json", "delivery-manifest.csv", "study-results.md",
        "manuscript-claim-audit.csv",
    }
    assert validate_release(output, "study", split_sha) == manifest

    (output / "study-results.md").write_text("corrupted\n", encoding="utf-8")
    with pytest.raises(ValueError, match="wrong hash"):
        validate_release(output, "study", split_sha)
