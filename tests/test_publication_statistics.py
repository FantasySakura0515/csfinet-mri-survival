...
from csfinet_repro import analysis_report
...
def test_publication_intervals_match_manuscript_report_despite_iou_and_row_order(tmp_path, variant, offset):
    ...
    for model,shift in [('csfinet',.02),('unet',0)]:
        ...
        paths.append(path)
        name,values=analysis_report.load_segmentation_metrics(path);models[name]=values
    expected=analysis_report._segmentation_rows([dict(name='synthetic',role='synthetic',models=models)],itertools.count(20260914+offset),100)
