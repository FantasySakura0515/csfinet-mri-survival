"""Synthetic-data tests for the v2 comparison reporter; no GPU, no real results, deterministic fixtures."""
...
from csfinet_repro.report_v2 import (MEASURES, REGIONS, SEGMENTATION_COLUMNS, SURVIVAL_MODELS, VARIANT_COLUMNS,
                                     build_report, main)
...
OUTPUTS = ("segmentation_comparison.csv", "segmentation_comparison.md", "segmentation_paired_tests.csv",
           ...
           "survival_degeneracy.csv", "manuscript-replacements-v2.md", "report-manifest.json")


def dice_value(model, tag, index):
    """CSFINet minus U-Net alternates +/-0.02 (null on average); v2 adds 0.05 with small patient-level jitter."""
    value = .55 + .3 * index / 46
    if model == "unet":
    ...
    for index, patient in enumerate(patients):
        for region, offset in REGION_OFFSET.items():
            ...
            if variant == "lcc":
                ...
                dice -= .01 + .002 * ((index % 3) - 1)
            blank = model == "csfinet" and tag == "v2" and (patient, region) == BLANK
...
def build_root(root, *, segmentation_tags=("v2",), variant_tags=("v2",), survival_tags=("v2mri",),
               adopted="lcc"):
    root = Path(root)
    ...
    for relative in ("configs/reconstruction-v21.json", "configs/survival-v2.json",
...
def test_full_report_recovers_constructed_directions_and_writes_every_file(tmp_path):
    ...
    assert manifest["adopted_variants"] == {"v2": "lcc"}
    ...
    assert {key[0] for key in table} == {"v2", "v2_lcc"} and len(table) == 12
    assert table[("v2", "csfinet", "ET")]["precision_n"] == "46"
    assert table[("v2", "csfinet", "ET")]["dice_n"] == "47"
    ...
    null = primary["A:v2:csfinet_minus_unet"]
    assert float(null["ci_low"]) < 0 < float(null["ci_high"]) and float(null["holm_p"]) > .05
    variant = primary["C:v2:csfinet:lcc_minus_raw"]
    ...
    assert len(survival) == 6 and {r["experiment"] for r in survival} == {"v2mri"}
    ...
    text = (out / "manuscript-replacements-v2.md").read_text(encoding="utf-8")
    ...
    assert {e["name"] for e in manifest["experiments"]["segmentation"]} == {"v2", "v2_lcc"}
...
def test_missing_survival_inputs_are_listed_without_other_experiments(tmp_path):
    ...
    manifest = build_report(root, "results/delivery/v2", seed=5, n_resamples=200)
    out = root / "results/delivery/v2"
    assert manifest["status"] == "completed_with_missing_inputs"
    missing = {item["path"]: item for item in manifest["missing_inputs"]}
    assert missing["results/survival/v2mri/survival_predictions.csv"]["effect"] == "experiment_unavailable"
    assert missing["results/tables/v2mri/survival/table7.json"]["effect"] == "cross_check_skipped"
    ...
    with pytest.raises(FileExistsError):
        build_report(root, "results/delivery/v2", seed=5, n_resamples=200)
...
def test_inconsistent_inputs_raise_before_anything_is_written(tmp_path):
    ...
    (root / "results/tables/v2/segmentation/table5.json").unlink()
    write_csv(root / "results/segmentation/unet-test-v2/segmentation_patient_metrics.csv",
              metric_rows("unet", "v2", patients=[*PATIENTS[:-1], "BraTS20_Training_999"]), SEGMENTATION_COLUMNS)
    ...
    rows = survival_rows("v2mri", patients=[*PATIENTS[:-1], "BraTS20_Training_999"])
    write_csv(root / "results/survival/v2mri/survival_predictions.csv", rows, list(rows[0]))
    write_json(root / "results/tables/v2mri/survival/table7.json", table7_json(rows))
    with pytest.raises(ValueError, match="different patients"):
        build_report(root, tmp_path / "out2", seed=5, n_resamples=100)

    root = build_root(tmp_path / "truth")
    rows = survival_rows("v2mri")
    rows[0]["y_true"] += 1.0
    write_csv(root / "results/survival/v2mri/survival_predictions.csv", rows, list(rows[0]))
    write_json(root / "results/tables/v2mri/survival/table7.json", table7_json(rows))
    ...
    path = root / "results/inference-variants/v2/test/test_patient_metrics.csv"
    ...
    path = root / "results/tables/v2/segmentation/table5.json"
...
def test_cli_main_is_deterministic_for_a_fixed_seed(tmp_path, capsys):
    root = build_root(tmp_path / "project", segmentation_tags=("v2",), variant_tags=(), survival_tags=("v2mri",))
    ...
    assert "missing (experiment_unavailable): results/inference-variants/v2/validation/decision.json" in printed
