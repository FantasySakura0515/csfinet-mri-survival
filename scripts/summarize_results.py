...
def summarize(results, output):
    ...
    for variant in ('raw', 'tta'):
        paths = [results / (f'segmentation/{m}-test-v2/segmentation_patient_metrics.csv' if variant == 'raw' else f'inference-variants/v2/test/per-variant/{m}_tta_patient_metrics.csv') for m in ('csfinet', 'unet')]
        ...
        experiments.append(dict(name='v2' if variant=='raw' else 'v2_tta', tag='v2', variant=None if variant=='raw' else 'tta', role='raw_inference' if variant=='raw' else 'validation_selected_tta', models=models, complete=True))
