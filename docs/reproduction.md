...
The downloader uses the tracked `release-manifest.json` and this repository's [v2.0.0 Release](https://github.com/FantasySakura0515/csfinet-mri-survival/releases/tag/v2.0.0), verifies checksums, and extracts into `runs/`. `--component segmentation` and `--component survival` select a subset. Checkpoints include the CSFINet/U-Net segmentation models and the four final survival configurations: `image`, `image_age`, `image_resection`, and `image_age_resection`.
